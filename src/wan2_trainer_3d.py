...
class WorldWanderTrainSystem3d(L.LightningModule):
    ...
    def process_data(self, batch, batch_idx):
        ...
        ego_depth_values = batch["ego_depth_pixel_values"]
        exo_gt_depth_pixel_values = batch["exo_gt_depth_pixel_values"]
        ...
        # depth video
        ego_depth_values = self.vae.encode(ego_depth_values).latent_dist.sample() # [B, C, F, H, W]
        ego_depth_values = (ego_depth_values - self.latents_mean) / self.latents_std # scaling
        
        exo_gt_depth_pixel_values = self.vae.encode(exo_gt_depth_pixel_values).latent_dist.sample() # [B, C, F, H, W]
        exo_gt_depth_pixel_values = (exo_gt_depth_pixel_values - self.latents_mean) / self.latents_std # scaling
        
        exo_warped_depth_pixel_values = self.vae.encode(exo_warped_depth_pixel_values).latent_dist.sample() # [B, C, F, H, W]
        exo_warped_depth_pixel_values = (exo_warped_depth_pixel_values - self.latents_mean) / self.latents_std # scaling
        ...
        return (
            ...
            prompt_embeds,
            ego_depth_values,
            exo_gt_depth_pixel_values,
            exo_warped_depth_pixel_values
        )
    ...
    def training_step(self, batch, batch_idx):
        (
            ...
            ref_pixel_values, 
            prompt_embeds, 
            ego_depth_pixel_values, 
            exo_gt_depth_pixel_values, 
            exo_warped_depth_pixel_values
        ) = self.process_data(batch, batch_idx)   
        ...
        if self.hparams.dataset.is_one2three:
            ...
            # in context 拼接 condition
            condition = torch.cat([first_pixel_values, exo_gt_depth_pixel_values], dim=2) # B, C, 2, H, W
            
            # condition
            attention_kwargs = {
                'encoder_condition_states': condition,
...
def main(opt):
    ...
    # custom system
    system = WorldWanderTrainSystem3d(opt)
    # custom logger
