...
class FlowMatchScheduler():
    ...
    def add_noise(self, original_samples, noise, timestep):
        timestep_id = torch.argmin((self.timesteps.unsqueeze(1).to(timestep.device) - timestep.unsqueeze(0)).abs(), dim = 0)
        # breakpoint()
        sigma = self.sigmas.to(timestep.device)[timestep_id].to(dtype=original_samples.dtype)
